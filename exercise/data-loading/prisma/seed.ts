...
async function createEmployeePositions() {
  ...
  for (const employeePosition of employeePositions) {
    await prisma.employeePosition.create({ data: employeePosition });
  }
}
...
async function createMachineTypes() {
  ...
  for (const machineType of machineTypes) {
    await prisma.machineType.create({ data: machineType });
  }
}
...
async function createJobSteps() {
  ...
  for (const jobStep of jobSteps) {
    await prisma.jobStep.create({ data: jobStep });
  }
}
...
async function createJobTaskStatuses() {
  ...
  for (const jobTaskStatus of jobTaskStatuses) {
    await prisma.jobTaskStatus.create({ data: jobTaskStatus });
  }
}
...
async function createJobTasks() {
  ...
  for (const jobTask of jobTasks) {
    await prisma.jobTask.create({ data: jobTask });
  }
}
